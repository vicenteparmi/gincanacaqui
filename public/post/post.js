...
function sendPost() {
  ...
  firebase.database().ref('posts').limitToFirst(1).once('child_added').then(function(s) {
    const nextPost = Number(s.key)-1;
    const nextRef = firebase.database().ref('posts/'+nextPost);
